...
class System_Page_Utilization_Query(Request):
    def __init__(self,
                 **kwargs):
        super(System_Page_Utilization_Query, self).__init__(**kwargs)
    ...
    def unpack(self, buf):
        ...
        sis = b2s(buf[offset:offset + alen])
        offset += alen

        # total_paging_pages (string,1-8,char10 plus 'K')
        self._total_paging_pages, _, sis = sis.partition(" ")

        # total_paging_pages_in_use (string,1-8,char10)
        self._total_paging_pages_in_use, _, sis = sis.partition(" ")

        # total_paging_percent_used (string,1-3,char10)
        self._total_paging_percent_used, _, sis = sis.partition(" ")

        for paging_volume in sis[:-1].split("\x00"):
            entry = Obj()
            self._paging_volume_array.append(entry)

            paging_volume = paging_volume.split(" ")

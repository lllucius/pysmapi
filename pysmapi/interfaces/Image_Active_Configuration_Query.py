...
class Image_Active_Configuration_Query(Request):
    ...
    MB = 2
    GB = 3
    memory_unit_names = {KB: "KB", MB: "MB", GB: "GB"}

    # Share type
    RELATIVE = 1
    ABSOLUTE = 2
    share_type_names = {RELATIVE: "RELATIVE", ABSOLUTE: "ABSOLUTE"}
    ...
    CHECK_STOPPED = 3
    NON_BASE_ACTIVE = 4
    cpu_state_names = {BASE: "BASE", STOPPED: "STOPPED", CHECK_STOPPED: "CHECK_STOPPED", NON_BASE_ACTIVE: "NON_BASE_ACTIVE"}
    ...
    PRT = 4
    DASD = 5
    device_type_names = {CONS: "CONS", RDR: "RDR", PUN: "PUN", PRT: "PRT", DASD: "DASD"}
    ...
    def unpack(self, buf):
        ...
        self._share_value = b2s(buf[offset:offset + alen])
        offset += alen
        ...
        while alen > 0:
            ...
            entry.device_address = b2s(buf[offset:offset + nlen])
            offset += nlen

...
class Asynchronous_Notification_Query_DM(Request):
    # Entity type
    DIRECTORY = 1
    entity_type_names = {DIRECTORY: "DIRECTORY"}

    # Subscription type
    INCLUDE = 1
    EXCLUDE = 2
    subscription_type_names = {INCLUDE: "INCLUDE", EXCLUDE: "EXCLUDE"}

    # Communication type
    TCP = 1
    UDP = 2
    communication_type_names = {TCP: "TCP", UDP: "UDP"}

    # Encoding
    UNSPECIFIED = 0
    ASCII = 1
    EBCDIC = 2
    encoding_names = {UNSPECIFIED: "UNSPECIFIED", ASCII: "ASCII", EBCDIC: "EBCDIC"}
    ...
    def pack(self):
        ...
        buf = struct.pack(fmt,
                          ...
                          self._port_number,
                          ip_len,
                          s2b(self._ip_address),
                          self._encoding,
                          sub_len,
                          s2b(self._subscriber_data))
    ...
    def unpack(self, buf):
        ...
        while alen > 0:
            ...
            entry.userid = b2s(buf[offset:offset + nlen])
            offset += nlen

            # subscription_type (int1)
            entry.subscription_type = buf[offset]
            offset += 1

            # communication_type (int1)
            entry.communication_type = buf[offset]
            offset += 1
            ...
            entry.ip_address = b2s(buf[offset:offset + nlen])
            offset += nlen

            # encoding (int1)
            entry.encoding = buf[offset]
            offset += 1
            ...
            # subscriber_data (string,0-64,charNA)
            entry.subscriber_data = b2s(buf[offset:offset + nlen])
            offset += nlen
